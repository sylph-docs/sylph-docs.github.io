...
# Sylph cookbook
  ...
  * [Database sketching options](#database-sketching-options)
      ...
      - [Converting to a two-stage database (.syl2db)](#converting-to-a-two-stage-database-syl2db)
...
- `-t`: parallelized sketching over the samples, and (as of sylph v1.0.0) within each sample too. E.g. with 5 samples and `-t 50`, each sample's sketching itself now uses several threads instead of just 1.
...
You can create a custom database with sylph (`syldb/syl2db`). This requires only fasta files. 
...
#### Converting to a two-stage database (`.syl2db`)

In sylph >= v1.0.0, we introduced the `syl2db` format. **This is most suitable for genomes with > 200kbp, such as prokaryotic genomes.** The key idea is to first query a sparse set of k-mers (`-c 3000` by default) to retrieve candidate genomes and then re-do profiling (`-c 200` by default) only on those candidates. 

`syl2db` is a drop-in replacement for the old database format but **it can be 2-10x faster and use ~5 times less memory**. 

```sh
sylph convert-db-two-screen bacteria.syldb -o bacteria_v2
```

- Converts an existing `.syldb` into the faster, lower-memory `.syl2db` format (sylph v1.0.0+). Same results as the original `.syldb`.
- Best for genomes >~200 kbp. Keep small genomes (viruses, plasmids) in a plain `.syldb` instead — you can mix `.syl2db` and `.syldb` in the same `profile`/`query` call.
- `--min-sparse-kmers`: lower this (default 50) if you have small genomes and you're okay losing some senstivity
...
```sh
sylph profile *.syldb *.syl2db *.sylsp  -t 50 -o results.tsv
# sylph query *.syldb *.syl2db *.sylsp  -t 50 -o results.tsv
```

- `*.syldb / *.syl2db`: all syldb or syl2db files (generated by sketching genomes) are aggregated together into one large database. 
- `*.sylsp`: each sylsp file (generated by sketching reads) is queried/profiled against the combined database.
- `-d`/`--databases`: optionally specify databases (`.syldb`/`.syl2db`) explicitly instead of relying on file extensions alone. 
...
sylph profile *.syl2db *.syldb *.sylsp -t 50 --min-number-kmers 20 -o results.tsv
```
...
- If you're using a `.syl2db` (two-stage) database, keep small genomes/viruses out of it and in a plain `.syldb` instead. You can mix `.syl2db` and `.syldb` databases together in the same `profile`/`query` call.
...
```sh 
sylph profile gtdb-r232-c200-dbv2.syl2db -1 read_name1.fq -2 read_name2.fq -o result.tsv
sylph-tax taxprof result.tsv -t GTDB_r232 -o PREFIX_
...
* `GTDB_r232` database is used, so we use `GTDB_r232` as the metadata file. See `sylph-tax taxprof -h` for available databases. 
...
```sh 
sylph profile  gtdb-r232-c200-dbv2.syl2db\
               fungi-refseq-2024-07-25-c200-v0.3.syldb \
              -1 A1.fq B1.fq -2 A2.fq B2.fq -o result.tsv
sylph-tax taxprof result.tsv -t GTDB_r232 FungiRefSeq-2024-07-25
...
* `-t` can take multiple taxonomies. This is needed because `gtdb-r232...` and `fungi-refseq...` are both used (these databases are concatenated by sylph into a big database for profiling). 
